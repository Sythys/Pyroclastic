...
impl ApplicationHandler for App {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        ...
        window_attributes.title = "Yellow Triangle".to_owned();
        let window = Arc::new(
        ...
        let pipeline = {
            ...
            let layout = PipelineLayout::new(
                self.device.clone(),
                PipelineDescriptorSetLayoutCreateInfo::from_stages(&stages).into_pipeline_layout_create_info(self.device.clone()).unwrap(),
            ).unwrap();
        ...
        match event {
            ...
            WindowEvent::RedrawRequested => {
                ...
                builder.begin_render_pass(RenderPassBeginInfo {
                    clear_values: vec![Some([0.2, 0.2, 0.8, 1.0].into())],
